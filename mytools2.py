...
class Tool():
    ...
    def search_model_names(self, string):
        #print(f"No model named '{string}', did you mean:")
        sorted_list = sorted([(model_name, fuzz.ratio(model_name,string)) for model_name in self.model_names], key=lambda i: i[1], reverse=True)
        display_list = [tup for tup in sorted_list if tup[1] >= 60]
        if len(display_list) < 3:
            display_list = sorted_list[:3]
        elif len(display_list) > 8:
            display_list = sorted_list[:9]
        for tup in display_list:
            print(f"\t{tup[0]}")

    def get_closest_modelname(self, string):
        if string in self.model_names:
            return string
        closest = sorted(self.model_names, key=lambda i: fuzz.ratio(i, string), reverse=True)[0]
        print(f"No model named '{string}' in system, using closest match '{closest}'")
        return closest
